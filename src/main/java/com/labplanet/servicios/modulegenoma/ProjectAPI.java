...
public class ProjectAPI extends HttpServlet {
    ...
    public enum  GenomaProjectAPIEndPoints{
          ...
          ;
        private GenomaProjectAPIEndPoints(String name, String mandatoryFields){
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            switch (actionName.toUpperCase()){
                case "PROJECT_NEW":
                    ...
                    dataSample =prj.createProject(schemaPrefix, token, projectName, fieldNames, fieldValues,  false);
                ...
                case "PROJECT_UPDATE":
                    ...
                    dataSample =prj.projectUpdate(schemaPrefix, token, projectName, fieldNames, fieldValues);
                    //logProgramSamplerSample(schemaPrefix, token, sampleTemplate, sampleTemplateVersion, fieldNames, fieldValues, programName, programLocation);
                    break;
                ...
                case "PROJECT_DEACTIVATE":
                    ...
                    if ("PROJECT_ACTIVATE".equalsIgnoreCase(actionName))
                        dataSample =prj.projectActivate(schemaPrefix, token, projectName);
                    else if ("PROJECT_DEACTIVATE".equalsIgnoreCase(actionName))
                        dataSample =prj.projectDeActivate(schemaPrefix, token, projectName);
                    else
                ...
                case "PROJECT_USER_DEACTIVATE":                           
                    areMandatoryParamsInResponse = LPHttp.areMandatoryParamsInApiRequest(request, GenomaProjectAPIEndPoints.PROJECT_ADD_USER.getMandatoryFields().split("\\|"));
                    if (LPPlatform.LAB_FALSE.equalsIgnoreCase(areMandatoryParamsInResponse[0].toString())){
                        LPFrontEnd.servletReturnResponseError(request, response, 
                                LPPlatform.API_ERRORTRAPING_MANDATORY_PARAMS_MISSING, new Object[]{areMandatoryParamsInResponse[1].toString()}, language);              
                        return;                  
                    ...
                    projectName=request.getParameter(GenomaProjectAPIParamsList.projectName.toString());
                    String userName=request.getParameter(GenomaProjectAPIParamsList.userName.toString());
                    String userRole=request.getParameter(GenomaProjectAPIParamsList.userRole.toString());
                    dataSample =prj.projectUserManagement(schemaPrefix, token, actionName, projectName, userName, userRole);
                    break;
                default:      
                    LPFrontEnd.servletReturnResponseError(request, response, LPPlatform.API_ERRORTRAPING_PROPERTY_ENDPOINT_NOT_FOUND, new Object[]{actionName, this.getServletName()}, language);              
                    return;                    
